...
app.controller('messageCtrl', ['$http', '$scope', '$timeout', 'wsMessage', function ($http, $scope, $timeout, wsMessage) {
    ...
    $scope.init = function (aid, oid, modal, deleteLink) {
        $scope.modal = modal;
    ...
    $scope.submit = function () {
        if ($scope.text) {
            var text=$scope.text;
            $scope.text = "";
            $http.post("/message?type=text&aid={0}&oid={1}".format($scope.aid, $scope.oid), text).success(function (data) {
    ...
    $scope.refreshMessages = function () {
        $http.get("/message?type=list&aid={0}&oid={1}".format($scope.aid, $scope.oid)).success(function (data) {
            ...
            jQuery.each($scope.messages, function () {
                this.deleteLink=$scope.deleteLink.replace("-1",this.id);
            });
        });
    };
