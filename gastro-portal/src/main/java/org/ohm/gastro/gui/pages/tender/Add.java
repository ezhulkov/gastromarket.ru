...
public class Add extends AbstractPage {
    ...
    @Component(id = "tenderInfoAjaxForm")
    private Form tenderInfoAjaxForm;
    ...
    public java.util.List<OrderEntity> getSamples() {
        return getOrderService().findAllTenders(RegionFilter.getCurrentRegion()).stream().limit(5).collect(Collectors.toList());
    }

    public void onPrepareFromTenderInfoAjaxForm() {
    ...
    public Object onSubmitFromTenderInfoAjaxForm() {
        photos = injectPhotos.getSubmittedPhotos();
        if (tenderInfoAjaxForm.getHasErrors() || mobile == null || tender.getDueDate() == null) {
            error = true;
