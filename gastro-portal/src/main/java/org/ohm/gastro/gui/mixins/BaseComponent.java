...
public abstract class BaseComponent {
    ...
    public static Optional<UserEntity> getAuthenticatedUser(UserService userService) {
        ...
        if (principal != null && principal instanceof UserEntity) {
            ...
            return Optional.ofNullable(userService == null ? user : userService.findUser(user.getId()));
        }
        return Optional.empty();
    }
    ...
    public Optional<Token> getToken(String code) {
        return Optional.ofNullable(getTokens().get(code));
    }
    ...
    protected <K, V> LoadingCache<K, V> createCache(Function<K, V> f, long duration, TimeUnit unit) {
